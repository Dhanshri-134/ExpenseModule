...
export function buildEstimateComputation(payload = {}) {
  ...
  return {
    ...
    summary: {
      ...
      totalManHours: projectSummary.totalManHours,
...
export async function loadEstimateGraph(admin, estimateIds = []) {
  ...
  for (const row of costCodeItems ?? []) {
    ...
    estimateRows.push({
      ...
      totalManHours: sumBy(splitEntries.laborEntries, (entry) => entry.totalManHours ?? entry.stHours + entry.otHours),
      totalDays: sumBy(splitEntries.laborEntries, (entry) => entry.totalDays ?? 0),
